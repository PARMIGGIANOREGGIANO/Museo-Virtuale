    ...
    public class MixedRealityInputModule : StandaloneInputModule, IMixedRealityPointerHandler, IMixedRealitySourceStateHandler
    {
        ...
        protected MouseState GetMousePointerEventDataForMrtkPointer(PointerData pointerData)
        {
            ...
            else
            {
                ...
                Vector3 proposedPosition = pointer.Position;

                // Controlla se sotto c'è terreno entro 10 unità
                if (UnityEngine.Physics.Raycast(proposedPosition, Vector3.down, out RaycastHit hit, 10f))
                {
                    // Aggiungi un piccolo offset verticale per stare sopra il terreno
                    proposedPosition.y = hit.point.y + 1.6f; // 1.6f è un'altezza approssimativa per la testa
                }
                else
                {
                    // In fallback, metti la camera leggermente più in alto
                    proposedPosition.y += 1.6f;
                }

                RaycastCamera.transform.position = proposedPosition;
